package cn.electric.middleware.db.router.config;

import cn.electric.middleware.db.router.DBRouterConfig;
import cn.electric.middleware.db.router.DBRouterJoinPoint;
import cn.electric.middleware.db.router.dynamic.DynamicDataSource;
import cn.electric.middleware.db.router.strategy.IDBRouterStrategy;
import cn.electric.middleware.db.router.strategy.impl.DBRouterStrategyHashCode;
import cn.electric.middleware.db.router.util.PropertyUtil;
import cn.electric.middleware.db.router.util.StringUtils;
...
public class DataSourceAutoConfig implements EnvironmentAware {
    ...
    public DBRouterConfig dbRouterConfig() {
        return new DBRouterConfig(dbCount, tbCount, groupCount, groupSize, routerKey);
    }
    ...
    private DataSource createDataSource(Map<String, Object> attributes) {
        try {
            ...
            DataSource ds = dataSourceProperties.initializeDataSourceBuilder().type((Class<DataSource>)Class.forName((String) attributes.get("type-class-name"))).build();
            // 用于访问数据源对象的元数据
    ...
    public void setEnvironment(Environment environment) {
        ...
        groupCount = Integer.parseInt(Objects.requireNonNull(environment.getProperty(prefix + "groupCount")));
        groupSize = Integer.parseInt(Objects.requireNonNull(environment.getProperty(prefix + "groupSize")));
